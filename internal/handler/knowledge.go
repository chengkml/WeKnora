...
func (h *KnowledgeHandler) CreateKnowledgeFromFile(c *gin.Context) {
	...
	// Detect ZIP files and route to the batch extraction handler.
	isZip := strings.HasSuffix(strings.ToLower(displayFileName), ".zip")

	if isZip {
		// ZIP upload: extract and create knowledge entries for each file.
		result, err := h.kgService.CreateKnowledgeFromZip(ctx, kbID, file, metadata, enableMultimodel, customFileName, tagIDs, channel, processOverrides)
		if err != nil {
		...
		logger.Infof(ctx, "ZIP knowledge created: %d files, kb_id: %s", len(result.Created), kbID)

...
func (h *AuthHandler) RegisterByInvite(c *gin.Context) {
	...
	if req.Token == "" || req.Email == "" || req.Username == "" || req.UserID == "" || req.Password == "" {
		c.Error(apperrors.NewValidationError("token, email, username, user_id and password are required"))
		return
	}

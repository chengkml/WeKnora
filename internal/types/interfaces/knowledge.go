...
type KnowledgeService interface {
	...
	// CreateKnowledgeFromZip extracts a ZIP archive from a multipart upload and
	// creates a knowledge entry for each compliant file inside it.
	// channel identifies the ingestion channel (e.g. "web", "api", "wechat"); empty defaults to "web".
	CreateKnowledgeFromZip(
		ctx context.Context,
		kbID string,
		file *multipart.FileHeader,
		metadata map[string]string,

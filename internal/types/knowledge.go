...
// ZipUploadResult holds the aggregate result of extracting and ingesting a ZIP archive.
type ZipUploadResult struct {
	Created          []string        `json:"created"`           // RelativePath of successfully created entries
	SkippedType      []string        `json:"skipped_type"`      // Files skipped due to unsupported type
	SkippedOversize  []string        `json:"skipped_oversize"`  // Files skipped due to per-file size limit
	SkippedDepth     []string        `json:"skipped_depth"`     // Nested ZIPs skipped due to max depth
	SkippedDuplicate []string        `json:"skipped_duplicate"` // Files skipped as duplicates
	SkippedCount     int             `json:"skipped_count"`     // Files skipped because max file count was reached
	Failed           []ZipFailedItem `json:"failed"`            // Files that caused unexpected errors
}

// ZipFailedItem records a file that caused an unexpected error during ingestion.
type ZipFailedItem struct {

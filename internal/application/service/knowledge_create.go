...
// CreateKnowledgeFromZip opens a ZIP archive from a multipart upload, extracts
// all compliant files recursively (up to 10 nesting levels), and creates a
// knowledge entry for each one.
func (s *knowledgeService) CreateKnowledgeFromZip(
	ctx context.Context,
	kbID string,
	...
	channel string,
	processOverrides *types.KnowledgeProcessOverrides,
) (*types.ZipUploadResult, error) {
	logger.Infof(ctx, "Start creating knowledge from ZIP, kbID=%s, filename=%s, size=%d", kbID, file.Filename, file.Size)

	// Open the uploaded ZIP.
	src, err := file.Open()
	if err != nil {
		return nil, werrors.NewBadRequestError("failed to open zip file").WithDetails(err.Error())
	}

	zipData, err := readAllAndClose(src, file.Size)
	if err != nil {
		return nil, werrors.NewBadRequestError("failed to read zip file").WithDetails(err.Error())
	}

	maxFileSize := secutils.GetMaxFileSize()

	opts := secutils.ExtractOptions{
		MaxDepth:         10,
		MaxFiles:         500,
		MaxTotalSizeBytes: 1 << 30, // 1 GB
		MaxTotalSizeRatio: 100,
	...
	extracted, summary, err := secutils.ExtractZipRecursively(reader, int64(len(zipData)), opts)
	if err != nil {
		return nil, werrors.NewBadRequestError("zip extraction failed").WithDetails(err.Error())
	}

	if len(extracted) == 0 {
		return nil, werrors.NewBadRequestError("ZIP 文件中未找到支持的文件类型")
	}

	// Map summary skips into result.
	result := &types.ZipUploadResult{
		SkippedType:     summary.SkippedTypes,
		SkippedOversize: summary.SkippedOversize,
		SkippedDepth:    summary.SkippedDepth,
		SkippedCount:    summary.SkippedCount,
	}

	// Get KB once (shared across all extracted files).
	kb, err := s.kbService.GetKnowledgeBaseByID(ctx, kbID)
	if err != nil {
		return nil, err
	}
	...
	for _, ef := range extracted {
		fileName := ef.RelativePath
		if fileName == "" {
			fileName = ef.Name
		}

		knowledge, createErr := s.createKnowledgeFromExtractedFile(
			ctx, kb, ef.Content, ef.Size, fileName, metadata, enableMultimodel, tagIDs, channel, processOverrides,
		)

		if createErr != nil {
			// Check for duplicate error — record and continue.
			if isDuplicateError(createErr) {
				result.SkippedDuplicate = append(result.SkippedDuplicate, ef.RelativePath)
				continue
			}
			// Other errors — record and continue.
			result.Failed = append(result.Failed, types.ZipFailedItem{
				RelativePath: ef.RelativePath,
				Error:        createErr.Error(),
			})
			logger.Warnf(ctx, "ZIP entry %q failed: %v", ef.RelativePath, createErr)
			continue
		}

		result.Created = append(result.Created, knowledge.FileName)
	}

	logger.Infof(ctx, "ZIP upload complete: created=%d, skipped_type=%d, skipped_oversize=%d, skipped_depth=%d, skipped_dup=%d, skipped_count=%d, failed=%d",
		len(result.Created), len(result.SkippedType), len(result.SkippedOversize), len(result.SkippedDepth), len(result.SkippedDuplicate), result.SkippedCount, len(result.Failed))

	return result, nil
}

// createKnowledgeFromExtractedFile creates a knowledge entry from an in-memory
// file extracted from a ZIP archive. It mirrors CreateKnowledgeFromFile but
// works with []byte instead of *multipart.FileHeader.
func (s *knowledgeService) createKnowledgeFromExtractedFile(
	ctx context.Context,
	...
	// Calculate hash from the in-memory data.
	h := md5.Sum(data)
	...
	fileSvc := s.resolveFileService(ctx, kb)
	filePath, err := fileSvc.SaveBytes(ctx, data, tenantID, knowledge.ID, false)
	if err != nil {
		return nil, err
	}
	knowledge.FilePath = filePath
	...
	if err := s.setAndAttachKnowledgeTags(ctx, tenantID, kb.ID, knowledge, tagIDs); err != nil {
		return nil, err
	}
	...
	payloadBytes, err := json.Marshal(taskPayload)
	if err != nil {
		logger.Errorf(ctx, "Failed to marshal task payload: %v", err)
		return nil, err
	}
...
// isDuplicateError checks whether the error is a DuplicateKnowledgeError.
func isDuplicateError(err error) bool {
	_, ok := err.(*types.DuplicateKnowledgeError)
	return ok
}

// readAllAndClose reads all bytes from a multipart.File and closes it.
func readAllAndClose(r io.ReadCloser, sizeHint int64) ([]byte, error) {
	defer r.Close()
	if sizeHint > 0 {

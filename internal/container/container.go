...
must(container.Provide(service.NewAgentBuildTaskService))
	...
	must(container.Provide(handler.NewMCPGatewayHandler))
	...
	must(container.Provide(handler.NewTokenizerHandler))
	...
	// Graph (Neo4j) service + handler
	must(container.Provide(service.NewGraphService))
	must(container.Provide(handler.NewGraphHandler))
	// IM integration

...
func RegisterSkillRoutes(r *gin.RouterGroup, skillHandler *handler.SkillHandler, g *rbacGuards) {
	skills := r.Group("/skills")
	{
		...
		// Export a skill as a ZIP — Viewer+ (read-only, same ZIP format as upload)
		skills.GET("/:name/export", g.Viewer(), skillHandler.ExportSkill)
...
func RegisterWikiPageRoutes(r *gin.RouterGroup, wikiHandler *handler.WikiPageHandler, g *rbacGuards) {
	...
	wikiRead := wiki.With(apiKeyRetrieve(apiKeyFullAccess()))
	{
		...
		// High-frequency keyword overview (aggregated, ranked by frequency)
		wikiRead.GET("/keywords/overview", g.Viewer(), g.KBAccessRead("kb_id"), wikiHandler.ListKeywordOverview)
	}
}

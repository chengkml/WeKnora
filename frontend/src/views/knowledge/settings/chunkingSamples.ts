...
const MARKDOWN_SAMPLE = `# WIKI 知识框架

WIKI 是一个基于 LLM 的开源企业知识框架，集 RAG 问答、ReAct 智能体、Wiki 知识图谱于一体。本文介绍其设计动机、架构与典型用法。

## 设计动机

企业内部知识散落在 Confluence、飞书、Notion、Git 仓库等多种系统，传统全文检索难以理解语义，单一 LLM 又缺少可信赖的上下文来源。WIKI 的目标是：
...
\`\`\`bash
git clone https://github.com/example/WIKI && cd WIKI
...
const FAQ_SAMPLE = `# WIKI 部署与使用 FAQ
...
\`\`\`
docker pull docker.m.daocloud.io/wechatopenai/wiki-app:v0.5.0
docker pull docker.m.daocloud.io/wechatopenai/wiki-docreader:v0.5.0
docker pull docker.m.daocloud.io/wechatopenai/wiki-ui:v0.5.0
\`\`\`

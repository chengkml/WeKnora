...
const executeUploadBatch = async (
  ...
  for (const file of files) {
    try {
      ...
      if (isSuccess) {
        // ZIP upload returns a structured result with created/skipped/failed arrays.
        const zipResult = responseData?.data?.created;
        if (file.name.toLowerCase().endsWith('.zip') && Array.isArray(zipResult)) {
          const created = zipResult.length;
          const skippedType = responseData?.data?.skipped_type?.length || 0;
          const skippedOversize = responseData?.data?.skipped_oversize?.length || 0;
          const skippedDuplicate = responseData?.data?.skipped_duplicate?.length || 0;
          const skippedDepth = responseData?.data?.skipped_depth?.length || 0;
          const skippedCount = responseData?.data?.skipped_count || 0;
          const failed = responseData?.data?.failed?.length || 0;
          const totalSkipped = skippedType + skippedOversize + skippedDuplicate + skippedDepth + skippedCount;
          successCount += created;
          if (totalSkipped > 0 || failed > 0) {
            const parts: string[] = [];
            if (created > 0) parts.push(t('knowledgeBase.zipExtractSuccess', { count: created }));
            if (skippedType > 0) parts.push(t('knowledgeBase.zipSkippedType', { count: skippedType }));
            if (skippedDuplicate > 0) parts.push(t('knowledgeBase.zipSkippedDuplicate', { count: skippedDuplicate }));
            if (skippedOversize > 0) parts.push(t('knowledgeBase.zipSkippedOversize', { count: skippedOversize }));
            if (skippedDepth > 0) parts.push(t('knowledgeBase.zipSkippedDepth', { count: skippedDepth }));
            if (skippedCount > 0) parts.push(t('knowledgeBase.zipSkippedCount', { count: skippedCount }));
            if (failed > 0) parts.push(t('knowledgeBase.zipFailed', { count: failed }));
            MessagePlugin.warning(parts.join('；'));
          } else {
            MessagePlugin.success(t('knowledgeBase.zipExtractSuccess', { count: created }));
          }
        } else {
          successCount++;
        }
